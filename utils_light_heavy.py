...
class ProteinSeqDataset(Dataset):
    ...
    def __init__(self, mode, seed=3227):
        ...
        self.lightX , self.lightY=[] , []
        self.heavyX, self.heavyY=[] , []
        ...
        for i in range(len(self.X)):
            if i%2==0:
                self.lightX.append(self.X[i])
                self.lightY.append(self.Y[i])
            
            else:
                self.heavyX.append(self.X[i])
                self.heavyY.append(self.X[i])
